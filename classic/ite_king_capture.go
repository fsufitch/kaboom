...
var IntentToEffect_KingCapture = kaboom.IntentToEffectRule{
	ID:          "king-capture-effect",
	Description: "Apply king capture intents as move + capture effects",
	Convert:     convertKingCaptureIntent,
}

func convertKingCaptureIntent(game kaboomstate.Game, intent kaboomstate.Intent) ([]*kaboomstate.Effect, error) {
	...
	if !ok {
		return nil, fmt.Errorf("%w: board %s not found for king capture intent", kaboom.ErrInvalidMove, pmProto.GetBoardUuid())
	}

	from := movement.From
	to := movement.To

	kingPiece, err := game.GetPieceAt(board.UUID(), from)
	if err != nil {
	...
	captureEffectProto := &kaboomproto.Effect{
		...
		Why:       fmt.Sprintf("king %s captures %s at %s", kingPiece.UUID(), targetPiece.UUID(), describePosition(to)),
	...
	moveEffectProto := &kaboomproto.Effect{
		Uuid:      kaboom.DefaultUUIDSource.NewUUID().String(),
		BoardUuid: board.UUID(),
		Why:       fmt.Sprintf("king %s moves to %s after capture", kingPiece.UUID(), describePosition(to)),
