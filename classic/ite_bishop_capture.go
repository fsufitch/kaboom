...
var IntentToEffect_BishopCapture = kaboom.IntentToEffectRule{
	ID:          "bishop-capture-effect",
	Description: "Apply bishop capture intents as move + capture effects",
	Convert:     convertBishopCaptureIntent,
}

func convertBishopCaptureIntent(game kaboomstate.Game, intent kaboomstate.Intent) ([]*kaboomstate.Effect, error) {
	...
	captureEffectProto := &kaboomproto.Effect{
		...
		Why:       fmt.Sprintf("bishop %s captures %s at %s", bishopPiece.UUID(), targetPiece.UUID(), describePosition(to)),
	...
	moveEffectProto := &kaboomproto.Effect{
		Uuid:      kaboom.DefaultUUIDSource.NewUUID().String(),
		BoardUuid: board.UUID(),
		Why:       fmt.Sprintf("bishop %s moves to %s after capture", bishopPiece.UUID(), describePosition(to)),
