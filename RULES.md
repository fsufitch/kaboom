...
# Kaboom Chess Rules (Draft)

Kaboom Chess is a variant that starts from the usual chess opening position and keeps the normal turn order and movement templates, but replaces classical captures with “Kaboom” interactions. Whenever a rule mentions a piece being “captured,” it means the piece was bumped or pushed off the board. The goal of the game is to capture the opposing king; there is no check or checkmate.

## Global Concepts

- **Bumping:** Many Kaboom moves push an opposing piece along the line of attack. A bump displaces the target exactly one square (two squares for the rook tackle). If that square is already occupied, the occupant is pushed as well, potentially causing a chain reaction. Any piece that would be pushed off the board is removed from play, but pieces may not “walk” off the board voluntarily—even under king control, a move must finish on the board.
- **Explosions/Novas:** Some special abilities remove the acting piece and affect surrounding or aligned pieces.
- **No En Passant:** Since captures are replaced with bumps, en passant does not apply.
...
The game ends immediately when a king is captured (for example, bumped off the board). If both kings are removed during the same turn (for instance, by a chain of bumps or a queen nova), the result is a double-loss. Stalemates are possible when neither player can force a king knockout with the remaining material, even after accounting for Kaboom abilities.
